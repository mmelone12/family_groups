class StaticPagesController < ApplicationController
  ...
  def home
    ...
    if signed_in?
      ...
      recurring_activities = Activity.where(['recurring = ? AND created_at >= ?', "yes", 30.days.ago(Time.now.to_date)]).near(@user, 100).order("RANDOM()").first(5)
      ...
      @recurring_activities = Activity.where(['recurring = ? AND created_at >= ?', "yes", 30.days.ago(Time.now.to_date)]).near(@city, 100).order("RANDOM()").first(5)
    end
  end
